...
- **`include/qcar_initial/critics/early_commit_critic.hpp`** / **`src/critics/early_commit_critic.cpp`**
  ...
  - Must live in `namespace mppi::critics` (not `qcar_initial::critics`):
...
(`rm -rf build/qcar_initial install/qcar_initial`) confirmed no stale object files carried the
...
`launch/qcar_initial.launch.py`: there was no Gazebo plugin publishing real joint states either -
...
- **`launch/qcar_initial.launch.py`**: removed the standalone `joint_state_publisher` node - it
...
- The final BT + `yaw_goal_tolerance` combination above was not re-verified live end-to-end after
  ...
  `ros2 launch qcar_initial qcar_nav2.launch.py` + goal send before trusting this fully.
...
- **`launch/qcar_initial.launch.py`**: removed the `joint_state_broadcaster`/
...
- **`disable_odom_tf` launch argument is currently a no-op.** It's still declared in
  `qcar_initial.launch.py` and passed through to `xacro`, and `qcar_slam.launch.py` still passes
...
- **`/joint_states` is self-referential and always reports zero.** `joint_state_publisher` in
  `qcar_initial.launch.py` is configured with `source_list: ['/joint_states']` - its own output
...
- **`launch/qcar_initial.launch.py`**: replaced the `drive_controller`/`steering_controller`
...
- **`launch/qcar_nav2.launch.py`** / **`launch/qcar_slam.launch.py`**: replaced the
  ...
  `qcar_initial.launch.py` with `disable_odom_tf:=true`, since Cartographer
...
- **Launch argument name collision**: the first attempt at the `disable_odom_tf` argument above
  ...
  `qcar_nav2.launch.py` includes `qcar_initial.launch.py` before including nav2's bringup, the
