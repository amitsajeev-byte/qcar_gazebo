...
| `launch/qcar_initial.launch.py` | Core sim bring-up: Gazebo, robot spawn, `robot_state_publisher`, controllers, RViz |
...
|---|---|---|
| `launch_sim` | `true` | Whether to include `qcar_initial.launch.py` (Gazebo + robot + controllers). Set to `false` if the simulation is already running (e.g. started separately, or you're driving real hardware) to avoid spawning a second instance. |
