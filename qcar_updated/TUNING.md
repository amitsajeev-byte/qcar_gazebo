...
| `minimum_turning_radius` | `0.7` m | Padded over the car's true ~0.445m minimum (wheelbase / tan(max steer)). Raised from `0.5` on 2026-07-19 (2) - `0.5` exactly matched `FollowPath.AckermannConstraints.min_turning_r` (the controller's own hard limit), which left MPPI's sampled trajectories zero margin: any sample noisier than perfect curves slightly tighter than 0.5m and gets penalized, so MPPI kept preferring straighter, cheaper trajectories over actually committing to a tight required curve - live-confirmed via a circle fit on a real `/plan` dump (fitted radius `0.49999999m`) that the robot froze on. `0.7` here vs `0.5` in the controller gives every planned curve real slack. Lower cautiously, and re-check this margin, if `max_steer`/the steering PID is later retuned to allow tighter turns. |
