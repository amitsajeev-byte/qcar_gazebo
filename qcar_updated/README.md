...
| `launch/qcar_updated.launch.py` | Core sim bring-up: Gazebo, robot spawn, `robot_state_publisher`, `joint_state_publisher`, RViz |
...
**Note:** `disable_odom_tf:=true` is *intended* to make `qcar_updated.launch.py` suppress the
...
|---|---|---|
| `launch_sim` | `true` | Whether to include `qcar_updated.launch.py` (Gazebo + robot). Set to `false` if the simulation is already running (e.g. started separately, or you're driving real hardware) to avoid spawning a second instance. |
