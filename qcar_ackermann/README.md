...
```bash
cd ~/ws/tmp/ros_ws
colcon build --packages-select qcar_ackermann
...
| `launch/qcar_ackermann.launch.py` | Core sim bring-up: Gazebo, robot spawn, `robot_state_publisher`, `joint_state_publisher`, RViz |
...
```bash
ros2 launch qcar_ackermann qcar_ackermann.launch.py
```
...
1. Launch SLAM (this also brings up the simulation):
   ...
   ```bash
   ros2 launch qcar_ackermann qcar_slam.launch.py
   ```
...
2. Drive the robot around the environment to build up the map. In a second terminal:
   ...
   ```bash
   ros2 run qcar_ackermann qcar_teleop_twist.py
   ```
...
4. Once the map looks complete, save it with `nav2_map_server`'s saver (subscribes to `/map`):
   ...
   ```bash
   ros2 run nav2_map_server map_saver_cli -f ~/ws/tmp/ros_ws/src/qcar_ackermann/maps/qcar_map
   ```

   This overwrites `maps/qcar_map.pgm` and `maps/qcar_map.yaml`, which is what
   `qcar_nav2.launch.py` loads by default.

**Note:** `disable_odom_tf:=true` is *intended* to make `qcar_ackermann.launch.py` suppress the
...
2. Launch navigation (this also brings up the simulation):
   ...
   ```bash
   ros2 launch qcar_ackermann qcar_nav2.launch.py
   ```
...
You can also drive manually at any time with `ros2 run qcar_ackermann qcar_teleop_twist.py`
...
|---|---|---|
| `launch_sim` | `true` | Whether to include `qcar_ackermann.launch.py` (Gazebo + robot). Set to `false` if the simulation is already running (e.g. started separately, or you're driving real hardware) to avoid spawning a second instance. |

`qcar_ackermann.launch.py` itself accepts:
...
```bash
ros2 launch qcar_ackermann qcar_slam.launch.py launch_sim:=false
```
