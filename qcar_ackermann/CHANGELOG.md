...
- **`include/qcar_ackermann/critics/early_commit_critic.hpp`** / **`src/critics/early_commit_critic.cpp`**
  ...
  - Must live in `namespace mppi::critics` (not `qcar_ackermann::critics`):
...
(`rm -rf build/qcar_ackermann install/qcar_ackermann`) confirmed no stale object files carried the
...
`launch/qcar_ackermann.launch.py`: there was no Gazebo plugin publishing real joint states either -
...
- **`launch/qcar_ackermann.launch.py`**: removed the standalone `joint_state_publisher` node - it
...
- The final BT + `yaw_goal_tolerance` combination above was not re-verified live end-to-end after
  ...
  `ros2 launch qcar_ackermann qcar_nav2.launch.py` + goal send before trusting this fully.
